...
module.exports = {
  ...
  async getFirstIng(req, res) {
    ...
    if (ingWithCount.length > 0) {
      firstIng = ingWithCount[0]._id;
    }else{
      firstIng = ''
    }
    res.json(firstIng);
  ...
  async getNextIng(req, res) {
    ...
    while (i < ingList.length) {
      ...
      for (let ingrediente of ingList[i].ingredientes) {
        if (
          req.body.ingDisponiveis.includes(ingrediente.nome) ||
          req.body.ingDisponiveis.length === 0
        ) {
          isValid = true;
        }
    ...
    const filteredValidIngredients = validIngredients.filter(function (e) {
      return !req.body.ingDisponiveis.includes(e);
    });
    if (filteredValidIngredients.length > 0) {
      ingCount = filteredValidIngredients.reduce(
        (a, c) => ((a[c] = (a[c] || 0) + 1), a),
        Object.create(null)
      );
      if (Object.keys(ingCount).length > 0) {
        nextIng = Object.keys(ingCount).reduce((a, b) =>
          ingCount[a] > ingCount[b] ? a : b
        );
      }
    }
    res.json(nextIng);
  ...
  async getRecipes(req, res) {
    const auxRecipes = await Receita.find({
